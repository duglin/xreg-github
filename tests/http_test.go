...
`)
	xHTTP(t, reg, "PUT", "/dirs/d1/files/f1/versions/v1$meta", `{ "id": "F1" }`, 400, `The "id" attribute must be set to "f1", not "F1"
`)
	xHTTP(t, reg, "PATCH", "/dirs/d1/files/f1/versions/v1$meta", `{ "id": "F1" }`, 400, `The "id" attribute must be set to "f1", not "F1"
`)
...
`)
	...
	// Resource
	xHTTP(t, reg, "POST", "/dirs/d1/files", `{"F1":{"id":"F1"}}`, 400, `Attempting to create a Resource with an "id" of "F1", when one already exists as "f1"
`)
	xHTTP(t, reg, "POST", "/dirs/d1/files", `{"f1":{"id":"F1"}}`, 400, `The "id" attribute must be set to "f1", not "F1"
`)
	xHTTP(t, reg, "POST", "/dirs/d1/files", `{"F1":{"id":"f1"}}`, 400, `Attempting to create a Resource with an "id" of "F1", when one already exists as "f1"
`)

	// Version
	xHTTP(t, reg, "POST", "/dirs/d1/files/f1/versions", `{"vv":{"id":"vv}}`, 400, `Error parsing json: unexpected EOF
`) // just a typo first
...
`,
	...
	xCheckHTTP(t, reg, &HTTPTest{
		...
		Code:       400,
		ResHeaders: []string{},
		ResBody: `The "id" attribute must be set to "ff1-proxy", not "v1"
`,
	})
